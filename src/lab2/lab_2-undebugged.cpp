...
int main(int argc, const char *argv[])
{
    ...
    cv::Mat image_cvt;
    cv::cvtColor(image, image_cvt, 12)
        ...
        for (int y = 0; y < image_size.height; y++)
    {
        ...
        ofs.seekp(y * fb_info.xres_virtual * fb_info.bits_per_pixel * 2);
        ...
        ofs.write(image_cvt.ptr(y), fb_info.xres_virtual * fb_info.bits_per_pixel * 2);
    }
...
struct framebuffer_info get_framebuffer_info(const char *framebuffer_device_path)
{
    ...
    int fd = open(framebuffer_device_path, O_RDWR);
        ...
        fb_info.bits_per_pixel =        screen_info.yres_virtual;
